...
class GetQuiz extends Component{
    constructor(props){
        ...
        this.state={
            quizData:[],
            userType:window.location.pathname.split("/").pop()
        }
    }
    componentDidMount(){
        console.log("In component did mount");
        fetch("http://localhost:3001/signin/:id/quiz/")
    ...
    update=(qid,changedQna)=>{
        fetch(`http://localhost:3001/signin/:id/quiz/${qid}`, 
        {  
            method: "PATCH",  
            headers: {    "Content-type": "application/json"  },  
            body: JSON.stringify({
    ...
    remove=(quizid)=>{
        fetch(`http://localhost:3001/signin/:id/quiz/${quizid}`, 
        { method: 'DELETE' })
        .then(res=>res.json())
        .then((result)=>{
            this.setState({
